<script setup lang="ts">
    const { authorName } = useRoute().params as { authorName: string };
    ...
    const onPageChange = async (newPage: number, limit: number) => {
        if (newPage === currentPage.value && limit === currentLimit.value && paginatedNovelsDataByAuthor.value && paginatedNovelsDataByAuthor.value.page != 0)
            return;
        try {
