...
const completedNovelCount = computed(() => {
  return Object.keys(novelStatuses.value).filter(
      novelId => novelStatuses.value[novelId] === 'updated' || novelStatuses.value[novelId] === 'skipped'
  ).length;
});
...
<template>
  <Container>
    ...
    <div>
      <!-- Import Button -->
      <Button :disabled="updatingNovels" @click="onSubmit" class="w-full md:w-auto">
    ...
    <section v-if="updatingNovels || showPreview" class="space-y-4">
      ...
      <div class="relative h-4 w-full overflow-hidden rounded-full bg-secondary">
        <div
            ...
            :style="{ width: `${(completedNovelCount / Object.keys(novelStatuses).length) * 100}%` }"
        ></div>
      </div>
      <p>Downloading: {{ completedNovelCount }} / {{ Object.keys(novelStatuses).length }}</p>
      ...
      <!-- Pagination -->
      <NumberedPaginator :totalPages="totalPages" :total="Object.keys(novelStatuses).length"
