...
const paginatedChapterStatuses = computed(() => {
  ...
  const sortedEntries = [...downloadingChapters, ...toDownloadChapters, ...errorChapters, ...completedChapters];
...
<template>
  <Container>
    ...
    <div>
      ...
      <!-- Import Button -->
      <Button :disabled="importingChapters" @click="onSubmit" class="w-full md:w-auto">
    ...
    <section v-if="importingChapters || showPreview" class="space-y-4">
      ...
      <div class="relative h-4 w-full overflow-hidden rounded-full bg-secondary">
        <div
          ...
          :style="{ width: `${(completedChaptersCount / Object.keys(chapterStatuses).length) * 100}%` }"
        ></div>
      </div>
      <p>Downloading: {{ completedChaptersCount }} / {{ Object.keys(chapterStatuses).length }}</p>
