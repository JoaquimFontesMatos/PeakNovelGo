...
<template>
  <header
    :class="[
      ...
      isTop
        ? 'border-opacity-50 bg-transparent'
        : 'bg-opacity-50 bg-primary backdrop-blur-md',
  ...
  >
    ...
    >
      <img
        ...
        alt="PeakNovelGo Logo"
        class="h-10 w-10 cursor-pointer brightness-105 hover:scale-105"
      />
      <h1 class="hidden text-2xl font-bold hover:cursor-pointer sm:block">
        PeakNovelGo
      </h1>
    </div>
    ...
    <div class="hidden flex-1 items-center justify-end gap-5 pr-4 md:flex">
      ...
      <div v-else class="flex items-center gap-5">
        ...
        <p
          @click="handleLogout()"
          class="cursor-pointer hover:text-accent-gold hover:underline"
        >
          Logout
        </p>
      </div>
    ...
    <button
      @click="toggleMenu()"
      class="hamburger mr-4 p-2 focus:outline-hidden md:hidden"
      aria-label="Toggle menu"
    >
      <Icon
        name="fluent:line-horizontal-3-20-filled"
        class="h-6 w-6"
        v-show="isMenuOpen"
      />
      <Icon
