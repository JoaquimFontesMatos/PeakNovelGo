<script setup lang="ts">
    ...
    const { cachedRecentlyVisitedNovels } = storeToRefs(novelStore);

    onMounted(async () => {
        if (import.meta.client) {
            await novelStore.getCachedNovels();
        }
    });
...
<template>
    <section>
        <h2 class="mb-6 text-3xl font-bold text-primary-content">Recently Visited</h2>
        <div v-if="cachedRecentlyVisitedNovels && cachedRecentlyVisitedNovels.length > 0" class="@container">
            <ul v-auto-animate class="grid grid-cols-1 justify-center gap-2 @md:gap-5 @lg:grid-cols-2 @xl:gap-8 @3xl:grid-cols-3 @6xl:grid-cols-4">
                <li
                    v-for="novel in cachedRecentlyVisitedNovels"
